...
/* Thread-local pointer to current context. Accessed by all functions that
   need parser state (lookup_var, define_func, etc.). */
static THREAD_LOCAL mp_context *current_ctx = NULL;

/* Helper to obtain the active context and error if missing */
static mp_context *ctx_get(void)
{
    if (!current_ctx)
    {
        fprintf(stderr, "Internal error: no parser context (did you call from outside exec?)\n");
    }
    return current_ctx;
}
...
/* ---------------- Dynamic Variable Table (context-backed) ---------------- */
...
/* set_var updates an existing var (if present) or creates a new non-const var */
...
/* define_func and lookup_func implement the API in parser_api.h.
   They operate on the current thread's context (current_ctx).
*/
...
static mp_result parse_primary(mp_parser *p)
{
    ...
    if (p->cur.kind == TK_IDENT)
    {
        ...
        if (accept(p, TK_LPAREN))
        {
            ...
            {
                /* existing trig handling unchanged - no zero-arg builtin here currently */
            }
...
{
    static THREAD_LOCAL int constants_initialized = 0;
...
/* exec creates a new context, sets it as the thread-local current_ctx,
   runs the parser on the given script, cleans up and returns the last
...
double exec(const char *script)
{
    ...
    /* Cleanup context-allocated memory */
    for (size_t i = 0; i < ctx_storage.n_funcs; i++)
        free(ctx_storage.funcs[i].body);
    free(ctx_storage.funcs);
    ctx_storage.funcs = NULL;
    ctx_storage.n_funcs = 0;
    ctx_storage.funcs_capacity = 0;

    free(ctx_storage.vars);
    ctx_storage.vars = NULL;
    ctx_storage.n_vars = 0;
    ctx_storage.vars_capacity = 0;

    /* Clear thread-local context pointer */
    current_ctx = NULL;
