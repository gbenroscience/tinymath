 ...
 * tinymath.c (v4)
 *
 * Full refactor: explicit context pointer threaded via mp_parser->ctx.
 * This file preserves the original features (variables, constants,
 * built-in functions, statistical functions, user-defined functions,
 * function definitions, df/diff integration with diff_module, symbolic mode,
 * parentheses-preserving symbolic expression formatting, etc.).
 *
 * Compile (Linux/macOS): gcc -O2 -std=c99 -Wall -lm -o math_parser tinymath.c
 *
 * Notes:
 * - parser_api.h and diff_module.h are provided in separate tabs (file blocks).
 * - The diff module's diff_func/diff_expr are included via diff_module.h and
 *   accept an explicit mp_context*.
 */
...
static double call_builtin(mp_context *ctx, const char *name, double *args, int n)
{
    ...
    if (strcmp(name, "sin") == 0 && n == 1)
        return sin((ctx ? (ctx->trig_mode==MODE_DEG ? args[0]*M_PI/180.0 : ctx->trig_mode==MODE_GRAD ? args[0]*M_PI/200.0 : args[0]) : args[0]));
    if (strcmp(name, "cos") == 0 && n == 1)
        return cos((ctx ? (ctx->trig_mode==MODE_DEG ? args[0]*M_PI/180.0 : ctx->trig_mode==MODE_GRAD ? args[0]*M_PI/200.0 : args[0]) : args[0]));
    if (strcmp(name, "tan") == 0 && n == 1)
        return tan((ctx ? (ctx->trig_mode==MODE_DEG ? args[0]*M_PI/180.0 : ctx->trig_mode==MODE_GRAD ? args[0]*M_PI/200.0 : args[0]) : args[0]));
...
double exec(const char *script)
{
    ...
    ctx_storage.constants_initialized = 0;

    mp_parser p = {{script, 0, strlen(script)}, {0}, &ctx_storage};
    advance(&p);
    double last_value = parse_program(&p);

    /* Cleanup */
    for (size_t i = 0; i < ctx_storage.n_funcs; i++)
        free(ctx_storage.funcs[i].body);
    free(ctx_storage.funcs);
    free(ctx_storage.vars);
