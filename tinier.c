...
{
    if (p && p->cur_arena)
        return arena_strdup(p->cur_arena, s);
    return strdup(s);
}
...
{
    if (p && p->cur_arena)
        return arena_strndup(p->cur_arena, s, n);
...
static void ast_free(ASTNode *n)
{
    if (!n)
        return;
    if (n->from_arena)
        return; /* arena-managed: don't free here */
    switch (n->type)
    {
    case NODE_NUM:
        free(n);
...
static void skip_ws(lexer_t *lx)
{
    for (;;)
    {
        while (lx->i < lx->len && isspace((unsigned char)lx->s[lx->i]))
            lx->i++;
        ...
        {
            lx->i += 2;
            while (lx->i + 1 < lx->len && !(lx->s[lx->i] == '*' && lx->s[lx->i + 1] == '/'))
                lx->i++;
            if (lx->i + 1 < lx->len)
                lx->i += 2;
            continue;
...
static int adopt(parser_t *p, tok_t k)
{
    if (p->cur.kind == k)
    {
        advance(p);
...
static double call_builtin(const char *name, double *args, int n, mp_context *ctx)
{
    ...
    if (strcmp(name, "sin") == 0 && n == 1)
        return sin(to_radians_ctx(ctx, args[0]));
    if (strcmp(name, "cos") == 0 && n == 1)
        return cos(to_radians_ctx(ctx, args[0]));
    if (strcmp(name, "tan") == 0 && n == 1)
        return tan(to_radians_ctx(ctx, args[0]));
...
static double eval_call(ASTNode *node, mp_context *ctx)
{
    ...
    {
        const char *m = ctx->trig_mode == 1 ? "Degrees" : ctx->trig_mode == 2 ? "Gradians"
                                                                              : "Radians";
    ...
    /* Save old var values / set parameters (stack allocation when small)
       Use lookup_var() to capture prior values and set_var() to assign params
       (set_var enforces const protection and will add variables if missing). */
    ...
    for (int i = 0; i < f->n_params; ++i) {
        double ov = 0.0;
        int found = lookup_var(ctx, f->params[i], &ov);
        had_old[i] = found ? 1 : 0;
        if (found) oldvals[i] = ov;

        /* Use set_var to assign the parameter. set_var enforces const protection and
           will add the variable if it didn't exist. If it fails (e.g. trying to
           assign to a const), an error message is printed by set_var and we continue. */
        if (!set_var(ctx, f->params[i], argvals[i])) {
            /* set_var prints an error (e.g. assigning to const) — continue but note failure */
        }
    }
    ...
    /* Restore old values or remove newly added params */
       /* Restore old values or remove newly added params.
       Prefer using set_var to restore previously existing variables so const
       semantics and internal bookkeeping remain consistent. If set_var fails
       for restoration, fall back to direct slot restore. */
    for (int i = 0; i < f->n_params; ++i) {
        if (had_old[i]) {
            /* Attempt to restore via set_var (respects const). If that fails,
               do a direct restore into the slot (best-effort fallback). */
            if (!set_var(ctx, f->params[i], oldvals[i])) {
...
static double eval_ast(ASTNode *n, mp_context *ctx)
{
    ...
    case NODE_VAR:
    {
        ...
        {
            if (strcmp(ctx->vars[i].name, n->ident) == 0)
...
static int parse_func_def_and_store(parser_t *p, const char *fname)
{
    ...
    if (!adopt(p, TK_EQ))
    {
        fprintf(stderr, "Error: Expected '=' to define function\n");
        return 0;
    }
    ...
    if (p->cur.kind == TK_IDENT)
    {
        ...
        advance(p);
           ...
           if (p->cur.kind == TK_EQ) {
            ...
            ast_free(expr);
            if (!isnan(val)) {
                /* use set_var helper to add/update variable (it enforces const rules) */
                if (!set_var(p->ctx, name, val)) {
    ...
    while (p->cur.kind != TK_END)
    {
        ...
        else
        {
            ...
            if (!stmt)
            {
                ...
                if (slen)
                    memcpy(small_buf, p->lx.s + stmt_start, slen);
                small_buf[slen] = '\0';
                used_small = 1;
            }
            else
...
void init_constants(mp_context *ctx)
{
    if (!ctx)
        return;
#ifdef M_PI
    set_const(ctx, "pi", M_PI);
#else
    set_const(ctx, "pi", 3.14159265358979323846);
#endif
...
void free_parsed_expr(mp_expr *e)
{
    if (!e)
        return;
    if (e->arena)
        arena_destroy(e->arena); /* frees all nodes & strings in arena */
    if (e->src)
        free(e->src);
    free(e);
}
...
void ctx_destroy(mp_context *c)
{
    ...
    for (size_t i = 0; i < c->n_funcs; ++i)
    {
        if (c->funcs[i].body)
            free(c->funcs[i].body);
        if (c->funcs[i].arena)
            arena_destroy((Arena *)c->funcs[i].arena);
    }
    if (c->scratch)
        arena_destroy(c->scratch);
    free(c->funcs);
    free(c->vars);
    free(c);
}
