...
# Of statements and scripts

A statement is a math expression, or a variable assignment or a function definition. A statement in tinymath must end with a semicolon.
...
A script is a combination of statements:

In C, a statement is expressed like this:

```C
const char* script = "2+2;"
```


In C, a script is expressed like this:
...
## Maximum speed:
The first law of achieving high speed with **tinymath** is to set `suppress_print` to 1. The only time you dont need this is when you are debugging stuff, for whatever reason. For all normal usage, pass 1 to the **exec_with_ctx** function and everywhere that `suppress_print` is needed. The `exec(const char *script)` method already has `suppress_print` set to 1. To execute a script or statement and see the results log, use `exec_with_log(const char *script)` The `suppress_print` field is set to true for this function and will run slower than the `exec(const char *script)` method.

The parser is plenty fast. On my machine, it does between 100 and 140 microseconds(0.1ms - 0.14ms) to evaluate:
```C
const char *script = "sin(1)+cos(1)+tan(1)+log(10)+sqrt(16)+exp(1)+pow(2,8)+abs(-42)+sum(1,2,3,4,5);";
```

On the way is an even faster version on the `speed-upgrade` branch which runs 
$sin(1)+cos(1)+tan(1)+log(10)+sqrt(16)+exp(1)+pow(2,8)+abs(-42)+sum(1,2,3,4,5)$

in about 5 microseconds(0.005ms) on my machine
...
The log_results and clear_results functions have been added.
...
// ... Initialize ...
mp_context *ctx = ctx_create();
int suppress_print = 1;

// --- Script 1 ---
exec_with_ctx(ctx, "a = 5 * 5; sin(a);", suppress_print);
log_results(ctx);   // Show results for Script 1
clear_results(ctx); // Wipe history but keep variables (a is still 25)

// --- Script 2 ---
exec_with_ctx(ctx, "a + 10;", suppress_print); 
log_results(ctx);   // Only shows "a + 10 => 35"
```